...
from libs.features.featuresExtractor import setParcelsWidthRef
from libs.features.heightEstimator import HeightEstimator
from libs.features.detection import detectAndFilterParcels
from libs.fasterObjectDetection.detector import ObjectDetector
from libs.vision.parcelAssociator import ParcelAssociator
from libs.vision.detectionTracker import DetectionTracker
from libs.motion.peer2peerTracker import Peer2peerTracker
from parcelSpace import TrackerSpace
...
class ParcelTracker():
    ...
    def __init__(self, configFile, trackerType='DEFAULT', logger=None):
        ...
        ## info pour dessiner zone tracking sur le convoyeur et sur l'image
    ...
    def _filterIncomingParcels(self, incomingParcels):
        ...
        if incomingParcels != [None]:
            filteredIncomingParcels = [parcel for parcel in incomingParcels if
                                       (parcel.isOutcoming and parcel.isExiting) and parcel.xPosition[1] > self.trackerSpace.xMinLimit]
    ...
    def _manageIncomingAndNewParcels(self, numObj, objects, unassociateDetections):
        ...
        if len(self.incomingParcels) != 0:
            ...
            for i in range(min(len(unassociateDetections2), len(self.incomingParcels))):
                resBoolean, area = self.trackerSpace.isInPrimeAssociationArea(
                    unassociatedObjects[unassociateDetections2[i]][2])
                if resBoolean:
    ...
    def _manageExitingParcels(self):
        ...
        for i in range(len(self.trackedParcels) - 1, -1, -1):
            if self.trackedParcels[i].numberOfTimesUndetected > self.numberOfTimeUndedectedThreshold:
                if self.logger is not None:
                    self.logger.info(
                        "Removing for overtaking numberOfTimeUndedectedThreshold : " + self.trackedParcels[i].parcelID)
    ...
    def decisionFonction(self):
        ...
        for parcel in self.trackedParcels:
            ...
            if parcel.isTracked and parcel.widthRef[0] != 0 and parcel.widthRef[1] != 0:
                ...
                areaCondition = (areaNew > (1 + self.areaConfidenceThreshold) * areaRef) or (
                            areaNew < self.areaConfidenceThreshold * areaRef)
